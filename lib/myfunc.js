...
exports.buffergif = async (image) => {
	...
	const filename = `${Math.random().toString(36)}`
			await fs.writeFileSync(`./XliconV4Media/trash/${filename}.gif`, image)
					 child_process.exec(
								`ffmpeg -i ./XliconV4Media/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./XliconV4Media/trash/${filename}.mp4`
										) 
  await sleepy(4000)
  
	var buffer5  =  await  fs.readFileSync(`./XliconV4Media/trash/${filename}.mp4`)
	Promise.all([unlink(`./XliconV4Media/video/${filename}.mp4`), unlink(`./XliconV4Media/gif/${filename}.gif`)])
	return buffer5
				   }
